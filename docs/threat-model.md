...
`nodesafe` is a **static analysis** scanner for plugin/node packages distributed
through ecosystems like ComfyUI (custom_nodes), LangFlow, Flowise, Node-RED, and
similar node-based workflow tools. This document defines what nodesafe defends
against and what it explicitly does not.

## STRIDE analysis

| Category | Vector | Mitigation in nodesafe |
|----------|--------|-------------------------|
...
## What nodesafe does NOT defend against
...
1. **Supply-chain compromise upstream of the plugin**. If a legitimate
   ...
   nodesafe detects the malware *when it reaches the plugin's code*, not at
   the original compromise.
2. **Runtime behavior**. nodesafe is static-only. It never executes scanned
...
3. **Sophisticated obfuscation specifically designed to evade nodesafe**.
...
4. **Malicious models or weights**. nodesafe scans code, not model files.
...
5. **Vulnerabilities in nodesafe itself**. Report via
...
6. **Active attacks on a compromised system**. If an attacker is already
   ...
   nodesafe's preventive value depends on integration with the Manager
   (so auto-installs are gated by scanning). nodesafe alone does not stop
...
### Scenario 3: Adversarial obfuscation specifically targeting nodesafe
...
defense. Without it, nodesafe protects only disciplined users. With it,
nodesafe becomes a gate on automated install flows.

## Assumptions

1. The host OS provides reasonable user-level isolation. nodesafe's role is
...
3. The signature database is current. Stale signatures = missed detections.
   `nodesafe update` should be run regularly. Future versions will support
...
- **Cryptographic signing of plugins** (a complementary problem; see the
  `.nodesafe` manifest design in V2+ for partial coverage)
